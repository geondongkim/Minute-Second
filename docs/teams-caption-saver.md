...
> 작성: 2026-04-27 | 버전: 2.0.0 | 기준: `manifest.json`, `content_script.js`, `service_worker.js`, `popup.js`
>
> **범위**: Manifest V3 구조, DOM 감지 전략, 저장 파이프라인, 사이드 패널 연동, AI 요약

---

## 1. 확장 개요

MS Teams 화상회의 중 라이브 캡션(Live Caption) 텍스트를 자동으로 캡처하고, AI로 요약하여 회의록을 생성하는 Chrome/Edge 확장입니다.

**지원 환경:**
- Chrome 114+ (Side Panel API 요구사항)
- Microsoft Edge (Chromium 기반)
- 대상 도메인: `teams.microsoft.com`, `teams.cloud.microsoft`, `teams.live.com`
...
| 자동저장 토글 | `chrome.storage.sync.set({ autoSave })` |
...
| `sync` | `autoSave` | 자동저장 ON/OFF (boolean) |
| `sync` | `autoEnableCaptions` | 자막 자동 켜기 (boolean) |
| `sync` | `trackAttendees` | 참석자 추적 (boolean) |
| `sync` | `saveOnEnd` | 회의 종료 시 저장 (boolean) |
| `sync` | `speakerAliases` | 발화자 별칭 목록 `[{ orig, alias }]` |
| `sync` | `aiProvider` | `gemini` / `openai` |
| `sync` | `geminiApiKey` | Gemini API 키 (평문 저장) |
| `sync` | `openaiApiKey` | OpenAI API 키 (평문 저장) |
| `sync` | `geminiModel` | 선택된 Gemini 모델명 |
| `sync` | `openaiModel` | 선택된 OpenAI 모델명 |
